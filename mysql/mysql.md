...
### 范式
- **第一范式**：每一列属性都是不可再分，确保每一列的原子性。
- **第二范式**：要求实体完全依赖于主关键字。
- **第三范式**：任何非主属性不会依赖其他的非主属性

范式化的优点：
- 范式化的更新操作通常比反范式化要快。
- 当数据较好地范式化时，就只有更少或没有重复数据，所以只需要修改更少的数据。
- 范式化的表通常更小，可以更好地放在内存里，所以执行操作会更快。
- 很少有多余的数据意味着检索列表数据时更少需要DISTINCT或者GROUP BY语句。

范式化设计的schema的缺点是通常需要关联。稍微复杂一点的查询语句在符合范式的schema上都可能需要至少一次关联。这不但代价昂贵，也可能使一些索引策略无效。

反范式化的优点：
- 可以很好地避免关联。
- 如果不需要关联表，则最坏的情况下是全表扫描，但是这也比关联要快。

最常见的反范式化是复制和缓存，在不同的表中存储相同的特定列。在MySQL5.0之后，可以使用触发器更新缓存值，这种方案变得更加简单。
...
- **索引下推**：默认开启
- **long_query_time**:开启让日志系统捕获所有查询。
- **max_sort_length**：设置排序的字符字节长度。
