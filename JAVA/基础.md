...
- size（）：多次不加锁统计，有异常再进行加锁统计。
 ...
 |名称|默认大小|扩容方式|线程安全|
 -|-|-|-
 |hashtable|11|2*n+1|通过方法加锁保证|
 |hashMap|16|2*n|不是线程安全|
 |TreeMap|||不是线程安全|
 |concurrentHashMap1.7|默认16 | 申请一个原来大小两倍的空间，不需要重新hash，可以进行分段扩容|是线程安全|
 |concurrentHashMap1.8|默认16|当table数量小于64时，先进行扩容，如果大于，再转红黑树|线程安全|
